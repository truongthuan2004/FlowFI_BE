...
public sealed class ImageTransactionService(
    ...
    IFinanceTransactionsClient financeClient) : IImageTransactionService
{
        ...
        CancellationToken cancellationToken)
    {
        var aiResult = await aiProcessingService.ProcessImageAsync(
            ...
            mockExtractedText,
            cancellationToken);
        var parsed = aiResult.ParsedData;
        ValidateParsedData(parsed);

        var transactionType = parsed.TransactionType!.ToUpperInvariant();
        var suggestedTag = CanonicalTag(parsed.Tag);
        var existingTags = await financeClient.ListTagsAsync(
            transactionType,
            bearerToken,
            cancellationToken);
        ...
        var title = BuildTitle(tag.Name, parsed.RawText);
    ...
    private static FinanceTagDto? FindMatchingTag(
        IEnumerable<FinanceTagDto> tags,
        string suggestedTag)
    {
        ...
        return tags
            ...
            {
                Tag = tag,
                Score = MatchScore(Normalize(tag.Name), normalizedSuggestion)
            })
    ...
    {
        if (tagName == suggestion)
        {
            return 100;
        }

        if (CanonicalTag(tagName) == CanonicalTag(suggestion))
        {
            return 90;
        }

        if (tagName.Contains(suggestion, StringComparison.Ordinal) ||
            suggestion.Contains(tagName, StringComparison.Ordinal))
        {
            return 60;
        }

        var commonTokens = tagName.Split(' ', StringSplitOptions.RemoveEmptyEntries)
